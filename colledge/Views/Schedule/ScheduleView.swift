...
struct ScheduleView: View {
    ...
    var body: some View {
        VStack {
            ScheduleDays(days: days, today: today, selectedDay: $selectedDay)
            if (scheduleItems.count == 0 && !loading && !error) || (!loading && !error && selectedDay != nil && scheduleItems[selectedDay!.idOfWeek - 1].count == 0) {
                ...
                ProgressView()
                Spacer()
            } else if selectedDay != nil && scheduleItems.count > 0 {
                ScrollView {
                    ScheduleList(items: scheduleItems[selectedDay!.idOfWeek - 1])
                }
            }
        }
